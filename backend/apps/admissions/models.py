...
from apps.academics.models import AcademicSession, Department, Program
...
class AdmissionApplication(models.Model):
    ...
    department = models.ForeignKey(Department, on_delete=models.PROTECT, related_name="admission_applications")
...
class ApplicationStatusHistory(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.application.application_number}: " f"{self.previous_status} → {self.new_status} at {self.timestamp}"
        )
...
class SeatMatrix(models.Model):
    ...
    academic_session = models.ForeignKey(AcademicSession, on_delete=models.CASCADE, related_name="seat_matrices")
...
class AdmissionAuditLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    application = models.ForeignKey(AdmissionApplication, on_delete=models.CASCADE, related_name="audit_logs")
