...
def test_academic_hierarchy_creation():
    ...
    prog = Program.objects.create(department=dept, name="Bachelor of Computer Applications", code="BCA", degree_level="UG")
...
def test_academic_session_single_current_rule():
    ...
    s1 = AcademicSession.objects.create(name="2024–2025", start_date="2024-08-01", end_date="2025-05-31", is_current=True)
    assert s1.is_current is True

    s2 = AcademicSession.objects.create(name="2025–2026", start_date="2025-08-01", end_date="2026-05-31", is_current=True)
    s1.refresh_from_db()
...
def test_academics_rest_api_crud():
    ...
    # 1. Create Faculty
    res_f = client.post(reverse("academics:faculty-list"), {"name": "Faculty of Commerce", "code": "FCOM"}, format="json")
    assert res_f.status_code == 201
    faculty_id = res_f.data["id"]

    # 2. Create Department
    res_d = client.post(reverse("academics:department-list"), {"faculty": faculty_id, "name": "Accounting & Finance", "code": "AF"}, format="json")
    assert res_d.status_code == 201
    dept_id = res_d.data["id"]

    # 3. Create Program
    res_p = client.post(reverse("academics:program-list"), {"department": dept_id, "name": "Bachelor of Commerce", "code": "BCom", "degree_level": "UG"}, format="json")
