...
from apps.examinations.models import Exam, ExamAttendance, ExamSchedule, ExamType, HallTicket, InvigilatorAssignment
...
class TestExaminationsModule:
    ...
    def setup_data(self):
        ...
        # Exam Type
        self.exam_type = ExamType.objects.create(name="End Semester Exam", code="END_SEM", category="end_semester", is_internal=False)
    ...
    def test_hall_ticket_and_exam_attendance(self):
        ...
        att = ExamService.mark_exam_attendance(str(schedule.id), str(self.student.id), status="present", actor=self.user)
